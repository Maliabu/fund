...
BankingWidgetSummary.propTypes = {
  ...
  total: PropTypes.any,
  btnColor: PropTypes.oneOf(['primary', 'secondary', 'info', 'success', 'warning', 'error']),
  action: PropTypes.string,
  showModal:PropTypes.func,
};

export default function BankingWidgetSummary({ title, showModal ,total, icon, percent, btnColor, color = 'primary', chartData, action }) {
  ...
  return (
    ...
    >
      ...
      <ReactApexChart type="area" series={[{ data: chartData }]} options={chartOptions} height={120} />
                      <Button fullWidth onClick={showModal} variant="contained" color={btnColor}>
          {action}
        </Button>
    </RootStyle>
  );
